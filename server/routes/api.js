...
router.post("/contactFormSubmit", function(req, res) {
  ...
  const verifyUrl = `https://www.google.com/recaptcha/api/siteverify?secret=${secretKey}&response=${gRecaptchaResponseValue}&remoteip=${
  ...
  request(verifyUrl, (err, response, body) => {
    ...
    if (body.success !== undefined && !body.success) {
      ...
      var mailOptions = {
        ...
        to: config.gmail.mailTo,
        subject:
          "New Corporate Event Form Submission from " +
          req.body.firstName +
          " " +
          req.body.lastName, // Subject line
        text: `From: ${req.body.firstName} ${req.body.lastName}
